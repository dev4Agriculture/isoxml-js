...
function stat(polygon: ExtendedPolygon) {
    return {
        outer: polygon.attributes.LineString
            .filter(lineString => lineString.attributes.LineStringType === LineStringLineStringTypeEnum.PolygonExterior).length,
        inner: polygon.attributes.LineString
            .filter(lineString => lineString.attributes.LineStringType === LineStringLineStringTypeEnum.PolygonInterior).length
    }
}
...
describe('Polygon Entity', () => {
    it('should be constructed from GeoJSON - version 3', async () => {
        ...
        const polygons = ExtendedPolygon.fromGeoJSON(MULTIPOLYGON.features[0].geometry as MultiPolygon, PolygonPolygonTypeEnum.TreatmentZone, isoxmlManager)
