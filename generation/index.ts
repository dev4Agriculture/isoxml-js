import {readFileSync, mkdirSync, writeFileSync, write, rmdirSync, rmSync, copyFileSync} from 'fs'
...
const entityTemplate = compile(
    readFileSync('./templates/Entity.hbs', 'utf-8'
))

const schema = xml2js(
    readFileSync('./xsd/ISO11783_Common_V4-3.xsd', 'utf-8'), 
...
const tags = elements.map((elem: ElementCompact) => {
    ...
    const attributes = elem['xs:complexType'][0]['xs:attribute'].map((attr: ElementCompact) => {
        try {
            ...
            const name = normalizeText(attr['xs:annotation'][0]['xs:documentation'][0]._text)
    ...
    return {
        tag, name, attributes, children: children
    }
})
...
// copyFileSync('./src/utils.ts', './dist/utils.ts')
